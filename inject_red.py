...
# RED PORT: target the sibling pokered decomp; reuse Yellow's pokecrystal donor.
PY = os.path.normpath(os.path.join(HERE, "..", "pokered"))
PC = os.path.join(HERE, "pokecrystal")
import gen2_data as _gd_early; _gd_early.PY = PY; _gd_early.PC = PC
...
import gen2_moves
gen2_moves.PY = PY; gen2_moves.PC = PC  # RED PORT: parse pokered, not pokeyellow
...
def apply_engine_patch():
    ...
    new = (
        '\t; RED PORT (Gen2 graft): gate the TM/HM-name shortcut on ITEM_NAME so\n'
        '\t; Pokemon/move/trainer indexes can exceed the HM01 ($C4) ceiling.\n'
        '\tld a, [wNameListType]\n'
        '\tcp ITEM_NAME\n'
        '\tjr nz, .notMachine\n'
...
def size_pokedex_wram():
    ...
    # pokered baseline: Stack org=$df00, stack `ds $100 - 1`. Move the org UP by
    # `grow` (frees that many bytes below it for the larger Pokedex arrays) and
    # shrink the stack the same amount so it still ends at $dfff.
    org = 0xdf00 + grow
    ...
    ws2 = re.sub(r"ds \$100 - \d+", f"ds $100 - {1 + grow}", ws, count=1)
    if ws2 != ws: write(wp, ws2)
    print(f"  sized Pokedex WRAM: NUM_POKEMON={num_pokemon}, +{grow}B "
          f"(Stack org=${org:04x}, stack=ds $100 - {1 + grow})")
...
def relocate_anim_overflow():
    ...
    for inc in moved:
        ...
        ms = ms.replace(inc, "", 1)
    # pokered's bank1E is the LAST section in main.asm, so append the new
    # floating ROMX section at EOF (the linker places it in any bank with room).
    if not ms.endswith("\n"):
        ms += "\n"
...
# CRITICAL: vanilla omits Mew (dex 151) from the BaseStats table (it is special-
# cased as MewBaseStats), but GetMonHeader/evos_moves index the table by
# (IndexToPokedex - 1) * BASE_DATA_SIZE, which assumes contiguous dex numbering.
# That works for dex 1-150, but once Gen2 mon (dex >= 152) are appended after
# Mewtwo, every one of them reads the NEXT species' entry (off by one) -> wrong
# types/stats AND wrong sprite pointer (garbled sprite). Fix: insert Mew at its
# dex-151 slot so the table is fully contiguous; (dex-1)*size is then correct for
# all index sites. data/pokemon/base_stats/mew.asm already exists (it is the file
# MewBaseStats:: includes), so we just include it a second time here.
print("base_stats:")
insert_before("data/pokemon/base_stats.asm", "\tassert_table_length NUM_POKEMON",
    'INCLUDE "data/pokemon/base_stats/mew.asm" ; dex 151 slot: keep table contiguous (GetMonHeader indexes by (dex-1)*size)\n'
    + "".join(f'INCLUDE "data/pokemon/base_stats/{m["f"]}.asm"\n' for m in MONS),
    "base_stats/chikorita.asm")
# Mew is now in-table, so the table is full length (was "NUM_POKEMON - 1 ; discount Mew").
_bsp = os.path.join(PY, "data/pokemon/base_stats.asm")
write(_bsp, read(_bsp).replace(
    "\tassert_table_length NUM_POKEMON - 1 ; discount Mew",
    "\tassert_table_length NUM_POKEMON ; Mew in-table (dex 151) for contiguous indexing"))
for m in MONS:
...
banktable = (
    ...
    'PicBankByIndex::\n' + picrows +
    '\nGetPicBankFar::\n'
    '; reads wCurPartySpecies (WRAM, survives bankswitch) ; out: e = pic ROM bank.\n'
    '; (Do NOT pass the index in a register: pokered Bankswitch does `ld bc,.Return`\n'
    ';  before jp hl, clobbering c — that scrambled the lookup -> garbled sprites.)\n'
...
if "GetPicBankFar" not in hs:
    ...
    new = (
        ...
        '\tcall Bankswitch ; GetPicBankFar reads wCurPartySpecies, returns bank in e\n'
        '\tld a, e\n'
        '\tjp UncompressSpriteData\n'
        # RED PORT: the vanilla resolver was 50 bytes; this far-call is 12. Shrinking it
        # would shift everything after it in HOME (incl. the tileset collision lists at
        # ~$1709), and pokered SAVES wTilesetCollisionPtr (a home address) -> old saves
        # would point at the wrong list -> collide everywhere. Pad to keep home layout
        # byte-stable so the collision lists stay at their stock addresses.
        '\tds 50 - (12), $00 ; preserve home layout (see comment)')
